			...
			<div class="row">
				<div class="col-sm-12">
					<div class="card-box table-responsive">
						...
						<hr>
						<button class="btn btn-primary btn-rounded m-b-20" data-toggle="modal" data-target=".modal-add"><i class="fa fa-plus-circle"></i> Tambah Poli</button>
			...
			<div class="modal-body" style="padding: 20px 50px 0 50px">
				<form method="POST" action="{{ url('admin/store/datapoli') }}">
					...
					<div class="form-group row">
						...
						<div class="col-sm-9">
							<input type="text" name="nama_poli" class="form-control" required="" autocomplete="off" placeholder="Nama Poli..">
						</div>
					</div>
					<div class="form-group row">
						...
						<div class="col-sm-9">
							<select name="dokter_id" class="form-control" required="">
								...
								foreach ($dokter as $dtr) {
									if ($dtr['id'] == old('dokter_id')) $select = 'selected';
									else $select = '';
					...
					<div class="form-group row">
						...
						<div class="col-sm-9">
							<textarea name="keterangan" class="form-control" required="" placeholder="Keterangan.." rows="3"></textarea>
						</div>
					</div>
					<div class="form-group row">
						...
						<div class="col-sm-9">
							<select name="status_layanan" class="form-control" required="">
								...
								foreach ($status_layanan as $jnk) {
									if ($jnk == old('status_layanan')) $select = 'selected';
									else $select = '';
				...
				<form method="POST" action="{{ url('admin/update/datapoli') }}">
					...
					<div class="form-group row">
						...
						<div class="col-sm-9">
							...
							<input type="text" class="form-control" required="" autocomplete="off" placeholder="Kode Poli.." value="{{ $dta->kode_poli }}" readonly="">
						</div>
					</div>
					<div class="form-group row">
						<label class="col-sm-3 col-form-label">Nama Poli</label>
						<div class="col-sm-9">
							<input type="text" name="nama_poli" class="form-control" required="" autocomplete="off" placeholder="Nama Poli.." value="{{ $dta->nama_poli }}">
						</div>
					</div>
					<div class="form-group row">
						<label class="col-sm-3 col-form-label">Dokter Penanggung Jawab</label>
					...
					<div class="form-group row">
						<label class="col-sm-3 col-form-label">Keterangan</label>
						<div class="col-sm-9">
							<textarea name="keterangan" class="form-control" required="" placeholder="Keterangan.." rows="3">{{ $dta->keterangan }}</textarea>
						</div>
					</div>
					<div class="form-group row">
						<label class="col-sm-3 col-form-label">Status Layanan</label>
						<div class="col-sm-9">
							<select name="status_layanan" class="form-control" required="">
								...
								$status_layanan = ['Aktif', 'Tidak Aktif'];
								foreach ($status_layanan as $jnk) {
									if ($jnk == $dta->status_layanan) $select = 'selected';
									else $select = '';
									echo '<option value="'.$jnk.'"'.$select.'>'.$jnk.'</option>';
								}
								@endphp
	...
	<div class="modal-dialog modal-sm" role="document">
		<div class="modal-content">
			...
			<div class="modal-footer form-inline">
				<a href="{{ url('admin/delete/datapoli/'.$dta->id) }}" role="button" class="btn btn-danger">Hapus</a>
