...
@section('content')
    ...
    $status = [];
    $jadwal_fix = [];
    foreach ($poli->where('status_layanan', 'Aktif')->get() as $i => $jdw) {
        $get_hari = [];
        $get_jam = [];
                        ...
                        <div class="card-box table-responsive">
                            ...
                            @if (isset($_GET['type']) && $_GET['type'] == 'new')
                                ...
                                <div class="text-center">
                                    <span class="">Silahkan daftar dan input data pasien baru terlebih
                                        dahulu!</span>
                                </div>
                                <form class="form-horizontal m-t-20" method="POST"
                                    action="{{ url('admin/store/pasien') }}">
                                    @csrf
                                    <div class="row">
                                        <div class="col-sm-6">
                                        ...
                                        <div class="col-sm-6">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-7">
                                                    <span class="text-dark">Info: Password default untuk pasien baru
                                                        adalah <b class="text-danger">BPPASS123</b></span>
                                    ...
                                    <div class="col-sm-6">
                                        <form method="POST" action="{{ url('admin/store/antrian') }}">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class="form-control select2" id="chg_user" name="user_id"
                                                    required="">
                                                    <option value="">.::Cari Data Pasien::.</option>
                                                    @foreach ($user->get() as $usr)
                                                        <option value="{{ $usr->id }}">
                                                            {{ $usr->nik . ' / ' . $usr->no_rekam_medik }}
                                                            ({{ $usr->nama }})</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" id="chg_poli" name="poli_id" required="">
                                                    ...
                                                    @foreach ($poli->where('status_layanan', 'Aktif')->get() as $i => $dta)
                                                        @if ($status[$i] == 'Buka')
                                                            <option value="{{ $dta->id }}">{{ $dta->nama_poli }}
                                                            </option>
                                                        @else
                                                            <option value="{{ $dta->id }}" disabled="">
                                                                {{ $dta->nama_poli }} <i>(Sedang Tutup)</i></option>
                                                        @endif
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="text-center m-t-20">
                                                <button type="submit" class="btn btn-rounded btn-primary"><i
                                                        class="fa fa-ticket"></i> Ambil Antrian</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
            ...
            function getAntrian(poli_id = null) {
                $.ajax({
                    ...
                    success: function(data) {
                        if (poli_id) {
                            $('#antrian-tersedia').html('<h1 class="text-center"><b>' + data +
                                '</b></h1>');
                        } else {
                            $('#antrian-tersedia').html(
                                '<h4 class="text-center"><i>--Pilih Poli--</i></h4>');
                        }
                    }
                });
